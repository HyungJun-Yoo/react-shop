...
import { cartsData } from "../store/store";
...
export default function Menu() {
  ...
  useEffect(() => {
    ...
    const jsTheme = document.querySelector(".js-theme") as HTMLInputElement;

    if (localDark === null) {
      setDark(true);
      jsTheme.checked = false;
    } else {
      if (localDark === "true") {
        setDark(true);
        jsTheme.checked = false;
    ...
    <>
      <div className="flex items-center px-2">
        <label className="swap swap-rotate mr-2 sm:mr-4">
          <input type="checkbox" className="js-theme" onClick={handleClick} />
          <svg
